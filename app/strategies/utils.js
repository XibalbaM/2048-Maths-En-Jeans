...
export function getRandomEmptyCell() {
    const empties = [];
    for (let r = 0; r < State.config.rows; r++) for (let c = 0; c < State.config.cols; c++) if (isEmptyCell(r, c)) empties.push([r, c]);
...
export function canMove(g) {
    for (let r = 0; r < State.config.rows; r++) for (let c = 0; c < State.config.cols; c++) {
        if (g[r][c] === 0) return true;
        if (c + 1 < State.config.cols && g[r][c] === g[r][c + 1]) return true;
        if (r + 1 < State.config.rows && g[r][c] === g[r + 1][c]) return true;
    }
    return false;
}